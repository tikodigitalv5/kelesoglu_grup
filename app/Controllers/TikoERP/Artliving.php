...
class Artliving extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->modelAltCategory = model($TikoERPModelPath . '\AltCategoryModel', true, $db_connection);
    ...
    public function b2bConnect()
    {
        ...
        $userDatabaseDetail = [
            ...
            'username' => 'artlivingsiparis_db',
            'password' => 'wV4UQyfZNptsd9ZYSaPB',
            'database' => 'artlivingsiparis_db',
    ...
    public function ArtlivingAddProduct(){
        try {
            // Veritabanı bağlantısı
            $database = $this->b2bConnect();
            
            // sistem3d alanı NULL olan tüm ürünleri al
            $ModelStock = $this->modelStock->where("status", "active")->findAll();
            ...
            foreach ($ModelStock as $Stock) {
                ...
                if($Stock["artliving"] > 0 || $Stock["artliving"] != '' ){

                    
                    $urun_ids = $Stock["artliving"];

                    $modelAltCategorys = $this->modelAltCategory->where("alt_category_id", $Stock['altcategory_id'])->first();
                    $data_kategoris = [
                       
                    'default_image'        => base_url($modelAltCategorys['default_image']),
                    ];
                    
                    $result = $database->table('alt_category')->set($data_kategoris)->where("app_id", $modelAltCategorys['alt_category_id'])->update();
            ...
            if (!$catBul) {
                throw new \Exception('Kategori bulunamadı.');
                    ...
                    if (!$kategoriSorgula || $kategoriSorgula == '') {
                        ...
                        $database->table('kategoriler_dil')->insert($data_urun_dil);
                    }
                    ...
                    ->where("urun_id", $urun_ids)
                    ->where("kategori_id", $kategori_id)
                    ->get()->getFirstRow();
                        if (!$urunlerKategori || $urunlerKategori == '' && isset($kategoriSorgula) ) {
                        if($kategoriSorgula){
                        $data_urun_kategori = [
                            'urun_id'        => $urun_ids,
                        ...
                        $database->table('urunler_kategori')->insert($data_urun_kategori);
                            ...
                            $database->table('urunler_kategori')->set($data_urun_kategori)->where("id", $urunlerKategori["id"])->update();
                        ...
                        // Güncellenecek verileri ayarlıyoruz
                        $data_urun = [
                            'paket'            => $Stock["paket"],
                            'stokkod'          => $Stock["stock_code"],
                            'ad'               => $Stock["stock_title"],
                            'renk'             => $Stock["altcategory_id"],
                            'sezon'            => $Stock["stock_id"],
                            'beden_dagilimi'   => $Stock["category_id"],
                            'img'              => "https://app.tikoportal.com.tr/" . $Stock["default_image"],
                        ];
                    
               
                    
                        // Kayıt olup olmadığını kontrol etme
                        $exists = $database->table('urunler')->where("sezon", $Stock['stock_id'])->countAllResults();
                        if (!$exists) {
                            continue;
                        } else {
                            // Eğer kayıt varsa güncelleme işlemi gerçekleştirilir
                            $insert = $database->table('urunler')->set($data_urun)->where("sezon", $Stock['stock_id'])->update();
                    
                            // Güncellemenin başarılı olup olmadığını kontrol etme
                            if ($insert) {
                                $gunc++;
                            }
                        }
            ...
            $altKategori = "";
            $altcatBul = $this->modelAltCategory->where("alt_category_id", $Stock["altcategory_id"])->first();
            if (!$altcatBul) {
                $altKategori = 0;
            }else{
                $altKategori = $altcatBul["alt_category_id"];
            }
            ...
            // İlk sorgu - Null kontrolü ekledik
$altCats = $this->modelAltCategory->where("alt_category_id", $Stock['altcategory_id'])->first();

if ($altCats) {
    $altKategoriSorgula = $database->table("alt_category")
        ->where("alt_category_title", $altCats['alt_category_title'])
        ->get()->getFirstRow(); // Sadece ilk satırı al
} else {
    $altKategoriSorgula = null;
}

// Eğer kategori yoksa yeni ekleme işlemi yap
if (!$altKategoriSorgula) {
    $modelAltCategorys = $this->modelAltCategory->where("alt_category_id", $Stock['altcategory_id'])->first();

    if ($modelAltCategorys) {
        $data_kategori = [
            'app_id' => $Stock['altcategory_id'],
            'alt_category_title' => $modelAltCategorys['alt_category_title'],
            'default_image' => base_url($modelAltCategorys['default_image']),
        ];

        // Yeni kategori ekle
        $database->table('alt_category')->insert($data_kategori);
        $altkategori_id = $database->insertID();
    } else {
        // Hata veya boş veri işlemi yapın
        $altkategori_id = null;
    }
} else {
    // Kategori zaten varsa, güncelleme işlemi yap
    $data_kategoris = [
        'default_image' => $altKategoriSorgula->default_image,
    ];

    $database->table('alt_category')
        ->set($data_kategoris)
        ->where("alt_category_id", $altKategoriSorgula->alt_category_id)
        ->update();

    // ID'yi al
    $altkategori_id = $altKategoriSorgula->alt_category_id;
}
            ...
            // Ürün sorgula
            $urunSorgula = $database->table("urunler")->where("stokkod", $Stock["stock_code"])->where("ad", $Stock["stock_title"])->get()->getFirstRow();
            ...
            if (!$urunSorgula || $urunSorgula == '' ) {
                ...
                if (!$urunlerTek) {
                    ...
                    $stock_recipe_itemss = $this->modelRecipeItem->join('stock_recipe', 'stock_recipe.recipe_id = recipe_item.recipe_id')
                    ...
                    ->where('stock_recipe.stock_id', $Stock["stock_id"])
                    ->findAll();
    
                    if(isset($stock_recipe_itemss) && $stock_recipe_itemss != ''  )
                    {
                       foreach($stock_recipe_itemss as $recipe_item)
                       {
                        $data_urun_tekk = [
                            'urun_id'        => $urun_id,
                            'paket'          => $recipe_item["paket"],
                            'barkod'         => $recipe_item["stock_barcode"],
                            'stokkod'        => $recipe_item["stock_code"],
                            'ad'             => $recipe_item["stock_title"],
                            'renk'           => $recipe_item["altcategory_id"],
                            'tema'           => $altKategori,
                            'adet'           => $recipe_item["total_amount"],
                            'sezon'          => $recipe_item["stock_id"],
                            'beden'          => $recipe_item["category_id"],
                            'asorti_barkod'  => $recipe_item["stock_barcode"],
            ...
            $urunKoleksiyon = $database->table("koleksiyonlar")
            ->where("app_id", $Stock["altcategory_id"])
            ->get()->getFirstRow();
            if($Stock["paket"] != 0){

                if (!$urunKoleksiyon || $urunKoleksiyon == '' && isset($urunKoleksiyon) ) {
                    ...
                    if($stock_recipe_items)
                    {
                        foreach($stock_recipe_items as $recipe)
                        {
                            $data_urun_kategori = [
                                'ana_paket_id'  => $Stock["stock_id"],
                                'urun_id'       => $recipe["stock_id"],
                                'app_id'        => $altkategori_id,
                                ];
                                $database->table('koleksiyonlar')->insert($data_urun_kategori);
                        }
                    }
            ...
            /* 
            $this->modelStock->set("artliving", $urun_id)->where("stock_id", $Stock["stock_id"])->update();

    
            try {
                $ftp_server = "78.135.107.25";
                $ftp_user = "b2b@siparis.artliving.com.tr";
                $ftp_pass = "Yxpk432*4";
                ...
                if (ftp_put($ftp_conn, $remote_file, $local_file, FTP_BINARY)) {
                    ...
                    $newGaleri = [
                        'img' => "https://siparis.artliving.com.tr/upload/image/" . $remote_file,
                    ];
            ...
            */
            ...
            if($gunc > 0)
            {
                echo json_encode(['icon' => 'success', 'message' => 'Mevcut Ürünler Artliving B2B ile Güncellendi']);
            }
            ...
            if ($count > 0) {
                echo json_encode(['icon' => 'success', 'message' => 'Toplam <b>' . $count . ' </b> Adet Ürün Artliving B2B Sitesine Aktarıldı.']);
            } 
            /* 
            else {
                echo json_encode(['icon' => 'info', 'message' => 'Artliving B2B sitesine aktarılacak ürün yoktur.']);
            }
            */
        } catch (\Exception $e) {
            echo json_encode([
            ...
            $b2bText = "";
             // *********** SİPARİŞ EKLEME BAŞLANGIÇ **************** //
                ...
                {   
                    ...
                    $saticiBilgileri = $musteriBilgileri;
                    ...
                    try {
                        ...
                        $b2bText.="Site: <b>http://siparis.artliving.com.tr/</b><br>Sipariş No: <b>" . $siparis["siparis_no"] . "</b> <br> Sipariş Tarihi : <b>" . date("Y/m/d H:i", strtotime($siparis["siparis_tarihi"]))."</b>"; 
                        ...
                        $order_note_id = 0;
                        $order_note = $siparis['siparis_not'];
                       ...
                       if ($stokBilgileri) {
                                ...
                                if(!empty($data_order_row["urun_sezon"])){
                                    ...
                                    $urunlerTek =  $database->table("urunler_tek")->where("sezon", $data_order_row["urun_sezon"])->get()->getResultArray();
                                    $urunlerTeks = $urunlerTek[0];
                                    

                                    $tumUrunler = $database->table("urunler_tek")->where("urun_id", $urunlerTeks["urun_id"])->get()->getResultArray();
                                    ...
                                    $insert_order_row_data = [
                                        ...
                                        'varyantlar' => $data_order_row['varyant'],
                                        'tax_id' => 0,
                                ...
                                foreach($tumUrunler as $keys){
                                    if($keys["id"] != $urunlerTeks["id"] && $urunlerTeks["paket"] == 1 ){
                                        $stokBilgileris = $this->modelStock->where('stock_id', $keys['sezon'])->first();

                                        $urunlerTek1 =  $database->table("urunler_tek")->where("sezon", $stokBilgileris["stock_id"])->get()->getResultArray();
                                        $urunlerTekss = $urunlerTek1[0];
                                        ...
                                        $insert_order_row_data = [
                                            ...
                                            'order_id' => $order_id,
                                            'paket'    => "1",
                                            'paket_text'=> "Paketli Ürün",
                                            'stock_id' => $stokBilgileris['stock_id'],
                                            'stock_title' => $stokBilgileris['stock_title'],
                                            'dopigo_title' => $stokBilgileris['stock_title'],
                                            'stock_amount' => $keys["adet"] * $data_order_row['adet_miktar'],
    ...
    $line = $trace[0]['line'] ?? 'N/A';
                        ...
                        echo json_encode(['icon' => 'error', 'message' => $line]);
                        return;
                    }
