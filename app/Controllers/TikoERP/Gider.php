...
class Gider extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->modelOffer = model($TikoERPModelPath . '\GiderModel', true, $db_connection);
        $this->modelOfferRow = model($TikoERPModelPath . '\GiderSatirModel', true, $db_connection);
    ...
    public function list($offer_status = 'all')
    {
        ...
        $offer_model = $this->modelOffer->join('money_unit', 'money_unit.money_unit_id = gider.money_unit_id')
            ->where('gider.user_id', session()->get('user_id'))
            ->orderBy('gider.offer_date','DESC');

            $cariler = $this->modelCari->findAll();

        if ($offer_status == 'all') {
            $page_title = "Tüm Giderler";
            $offer_items = "";
        } else if ($offer_status == 'open') {
            $page_title = "Açık Giderler";
            $offer_items = "";
        } else if ($offer_status == 'closed') {
            $page_title = "Kapalı Giderler";
            $offer_items = "";
        } else {
    ...
    public function quickOfferPrint($invoice_id)
    {
        ...
        $invoice_rows = $this->modelOfferRow->join('unit', 'unit.unit_id = offer_row.unit_id')
            ...
            ->join('offer', 'offer.gider_id = offer_row.gider_id')
            ->where('offer.gider_id', $invoice_id)
            ->findAll();
    ...
    public function setOfferStatus() {
        ...
        try {
            ...
            echo json_encode(['icon' => 'success', 'message' => 'Gider durumu başarıyla güncellendi.', 'offerId' => $offerId]);
            return;
        } catch (\Exception $e) {
    ...
    public function detail($gider_id)
    {
        ...
        $offer_item = $this->modelOffer->join('money_unit', 'money_unit.money_unit_id = giderler.money_unit_id')
        ->join('gider_category', 'gider_category.gider_category_id = giderler.gider_kategori')
            ->where('giderler.user_id', session()->get('user_id'))
            ->where('giderler.gider_id', $gider_id)
            ->orderBy('giderler.gider_date')
            ->first();
        ...
        $offer_rows = $this->modelOfferRow->join('stock', 'stock.stock_id = gider_satirlari.stock_id', 'left')
        ->join('unit', 'unit.unit_id = gider_satirlari.unit_id')
        ->join('giderler', 'giderler.gider_id = gider_satirlari.gider_id')
        ->select('stock.*,unit.*,giderler.*, gider_satirlari.*, gider_satirlari.stock_title as stok_adi')
        ->where('giderler.gider_id', $gider_id)
        ->orderBy('gider_satirlari.gider_satir_id', "ASC")
        ...
        $data = [
            'faturaVarmi' => 0,
            'invoice_item' => $offer_item,
            'invoice_rows' => $offer_rows
        ];

        // print_r($offer_rows);
        // return;

        return view('tportal/stoklar/giderler/detay', $data);
    }

    public function offerMovements($gider_id = null)
    {

        $offer_item = $this->modelOffer->join('money_unit', 'money_unit.money_unit_id = offer.money_unit_id')
            ->where('offer.user_id', session()->get('user_id'))
            ->where('offer.gider_id', $gider_id)
    ...
    public function create()
    {
        ...
        $transaction_prefix = "GDR";
        $errRows = [];
        if ($this->request->getMethod('true') == 'POST') {

            try {
                $data_form = $this->request->getPost('data_form');
                $data_offer_rows = $this->request->getPost('data_order_rows');
                $data_offer_amounts = $this->request->getPost('data_order_amounts');
              
          

                foreach ($data_form as $data) {
                    $key = $data['name'];
                    $value = $data['value'] ?? "boş";
                ...
                $transaction_amount = isset($data_offer_amounts['amount_to_be_paid']) 
                ? convert_number_for_sql($data_offer_amounts['amount_to_be_paid']) 
                : convert_number_for_sql($data_offer_amounts['amount_to_be_paid_try'] ?? 0);
                ...
                if (isset($new_data_form['chk_musteri_bakiye_ekle']) && $new_data_form['chk_musteri_bakiye_ekle'] == 'on') {
                    $gider_note_id = $new_data_form['invoiceNotesId'];
                    $gider_note = $new_data_form['txt_fatura_not'] . " - " . $cari_balance;
                } else {
                    $gider_note_id = $new_data_form['invoiceNotesId'];
                    $gider_note = $new_data_form['txt_fatura_not'];
                }
                ...
                if (isset($new_data_form['chk_not_kaydet']) && $new_data_form['chk_not_kaydet'] == 'on') {
                    ...
                    if (!$note_item) {
                        ...
                        $gider_note_id = $this->modelNote->getInsertID();
                    } else {
                ...
                //     'transaction_description' => $gider_note,
                ...
                $d_date = $new_data_form['fatura_tarihi'];
                $insert_offer_data = [
                    'user_id' => session()->get('user_id'),
                    'client_id' => session()->get('client_id'),
                    'money_unit_id' => $data_offer_amounts['money_unit_id'],
                    'gider_no' => $transaction_prefix . $create_offer_code,
                    'gider_date' => $offer_datetime,
                    'gider_kategori' => $new_data_form['gider_kategori'],
                    'aciklama' => '',
                    'gider_turu' => "fis_fatura",
                    'odeme_durumu' => $new_data_form["odeme_durumu"],
                    'kasa_id' => $new_data_form["kasa_sec"] ?? null,
                    
                    
                    'gider_note_id' => $new_data_form['invoiceNotesId'] == '' ? $gider_note_id : $new_data_form['invoiceNotesId'],
                    'gider_note' => $gider_note,
                    ...
                    'address' => $new_data_form['address'],
                    'fis_fatura_belge' => $new_data_form['fis_fatura_belge'],
                ...
                $gider_id = $this->modelOffer->getInsertID();
                ...
                foreach ($data_offer_rows as $data_offer_row) {
                    if (isset($data_offer_row['isDeleted'])) {
                        ...
                        $insert_offer_row_data = [
                            ...
                            'total_price' => convert_number_for_sql($data_offer_row['total_price']),
                        ];
                ...
                $transaction_title = "Gider girişinden oluşan hareket ";
                $transaction_description = "Gider girişinden oluşan hareket ";
       
                $transaction_amount = $data_offer_amounts['amount_to_be_paid'];
                # Henüz bu kısıma işlem ücreti eklemedik.
                $transaction_real_amount = convert_number_for_sql($transaction_amount);
                $transaction_date = date("Y-m-d H:i:s");
         

                $financial_account_id =  $new_data_form["kasa_sec"] ?? 0;
                ...
                if (!$financial_account_item && ($transaction_type != 'gider')) {
                ...
                $transaction_prefixs = "GDRTHSLT";
                $transaction_number = $transaction_prefixs . str_pad($transaction_counter, 6, '0', STR_PAD_LEFT);



                if ($new_data_form['odeme_durumu'] == 'odendi') {
                    ...
                    $insert_financial_movement_data = [
                        ...
                        'transaction_number' => $transaction_number,
                        'transaction_direction' => 'entry',
                        'transaction_type' => 'incoming_gider',
                        'gider_id' => $gider_id,
                        'transaction_tool' => 'not_payroll',
                        'transaction_title' => 'Gider girişinden yapılan tahsilat ',
                        'transaction_description' => $gider_note,
                        'transaction_amount' => convert_number_for_sql($transaction_amount),
                        'transaction_real_amount' => convert_number_for_sql($transaction_amount),
                        'transaction_date' => date("Y-m-d H:i:s"),
                        'transaction_prefix' => $transaction_prefix,
                        'transaction_counter' => $transaction_counter
                    ...
                    $update_financial_account_data = [
                        'account_balance' => $financial_account_item['account_balance'] + convert_number_for_sql($transaction_amount)
                    ];
                    ...
                    $this->modelOffer->update($gider_id, $update_collect_invoice_data);


                    $lastc = convert_number_for_sql($gider_id == 3 ? convert_number_for_sql($transaction_amount) : convert_number_for_sql($transaction_amount));
                    ...
                    $insert_financial_movement_data = [
                        ...
                        'transaction_type' => 'outgoing_gider',
                        'gider_id' => $gider_id,
                        'transaction_tool' => 'not_payroll',
                        'transaction_title' => 'Gider oluştur anında oluşan hareket',
                        'transaction_description' => $gider_note,
                        'transaction_amount' => convert_number_for_sql($transaction_amount),
                        'transaction_real_amount' => convert_number_for_sql($transaction_amount),
                        'transaction_date' => date("Y-m-d H:i:s"),
                    ...
                    $controller = new \App\Controllers\TikoERP\Cari();
                    $bakiyeHesapla = $controller->bakiyeHesapla($cari_id);
                ...
                echo json_encode(['icon' => 'success', 'message' => 'Gider başarıyla sisteme kaydedildi.', 'errRows' => $errRows, 'newOfferId' => $gider_id]);
                return;
            } catch (\Exception $e) {
            ...
            $kasalar = $this->modelFinancialAccount->join('money_unit', 'money_unit.money_unit_id = financial_account.money_unit_id')->findAll();
            $gider_kategori = $this->modelGiderKategori->where("status","active")->findAll();
            $gider_grup = $this->modelGiderGrup->where("status","active")->findAll();
            $data = [
                'page_title' => "Yeni Gider Oluştur",
                'Kurlar' => $Kurlar,
                'kasalar' => $kasalar,
                'gider_grup' => $gider_grup,
                'gider_kategori' => $gider_kategori,
            ...
            return view('tportal/stoklar/giderler/yeni', $data);
        }
    }
    ...
    public function getOfferList($order_status = 'all')
{
    ...
    ->where('giderler.user_id', session()->get('user_id'))
    ->where('giderler.deleted_at', null)
    ...
    if (!empty($start_date) && !empty($end_date)) {
        ...
        $builder->where('DATE(giderler.gider_date) >=', $start_date);
        $builder->where('DATE(giderler.gider_date) <=', $end_date);
    }

    // Durum filtresi
    $status = $this->request->getGet('status');
    if (!empty($status) && $status != '0') {
        $builder->where('giderler.offer_status', $status);
    }

    // Müşteri filtresi
    $musteri = $this->request->getGet('musteri');
    if (!empty($musteri) && $musteri != '0') {
        $builder->where('giderler.cari_id', $musteri);
    }
    ...
    if ($order_status == 'all') {
        ...
        $page_title = "Açık Giderler";
        $builder->groupStart()
            ->where('giderler.offer_status', 'new')
            ->orWhere('giderler.offer_status', 'pending')
            ->groupEnd();
    } else if ($order_status == 'closed') {
        $page_title = "Kapalı Giderler";
        $builder->groupStart()
            ->where('giderler.offer_status', 'failed')
            ->orWhere('giderler.offer_status', 'success')
            ->groupEnd();
    } else {
        return redirect()->back();
    }
    ...
    $data = DataTable::of($builder)
        ->setSearchableColumns(['gider_date', 'gider_no', 'cari_invoice_title'])
...
{
     // Dosya yükleme kurallarını tanımlayın
     $validationRule = [
        'fis_fatura_belge' => 'uploaded[fis_fatura_belge]|max_size[fis_fatura_belge,1024]|ext_in[fis_fatura_belge,pdf,jpg,jpeg,png,zip]'
    ];

    // Formdan gelen dosyayı kontrol et
    if (!$this->validate($validationRule)) {
        return redirect()->back()->withInput()->with('error', 'Dosya formatı geçersiz veya dosya çok büyük.');
    }

    // Dosya yükleme işlemi
    $file = $this->request->getFile('fis_fatura_belge');
    
    // public/uploads klasörüne kaydet
  
    $uploadPath = ROOTPATH . 'public_html/uploads/giderler';
    // Dosya adı oluştur
    $fileName = $file->getRandomName();

    // Dosyayı public/uploads klasörüne kaydedin
    if ($file->move($uploadPath, $fileName)) {
        // Dosya başarıyla yüklendi
        $filePath = 'uploads/giderler/' . $fileName;  // public klasöründe erişilebilen yolu belirleyin

        return $this->response->setJSON(['filePath' => $filePath]); // JSON formatında yol gönder
    }

    return $this->response->setJSON(['error' => 'Dosya yükleme sırasında hata oluştu.']);
}
    ...
    if(empty($offer_item["cari_id"])){
        ...
        if ($this->request->getMethod('true') == 'POST') {
            if (!$offer_item) {
                echo json_encode(['icon' => 'error', 'message' => 'Gider bulunamadı.']);
                return;
            }
            ...
            try {
                $data_form = $this->request->getPost("data_form");
                $data_offer_rows = $this->request->getPost("data_invoice_rows");

    
                $data_offer_amounts = $this->request->getPost("data_invoice_amounts");
                ...
                $update_offer_data = [
                    ...
                    'gider_turu' => "fis_fatura",
                    'odeme_durumu' => $new_data_form["odeme_durumu"],
                    'kasa_id' => $new_data_form["kasa_sec"],
                    
                    'gider_note_id' => $gider_note_id,
                    'gider_note' => $gider_note,
                    
                    'is_validity' => 0,
                    'validity_date' => convert_datetime_for_sql_time($offer_date, $offer_time),
                ...
                $this->modelOffer->update($gider_id, $update_offer_data);
                ...
                foreach ($data_offer_rows as $data_offer_row) {
                    ...
                    $gider_satir_id = $data_offer_row["gider_satir_id"];
                    
                    $this->modelOfferRow->where('user_id', session()->get('user_id'))
                    ->where('gider_satir_id', $gider_satir_id)
                    ->delete();
             

                    #STOK KONTROLÜ DAHA SONRA YAPILACAK
                    // $stock_item = $this->modelStock->where('user_id', session()->get('user_id'))->where('stock_id', $data_offer_row['stock_id'])->first();
                    // if (!$stock_item) {
                    //     $errRows[] = [
                    ...
                    $offer_row_data = [
                        ...
                        'stock_id' => $data_offer_row['stock_id'],
                        'stock_title' => $data_offer_row['stock_title'],
                        'stock_amount' => $data_offer_row['stock_amount'], //convert_number_for_sql($data_offer_row['stock_amount'])
                    ...
                    if ($gider_satir_id == 0) {
                        ...
                        $this->modelOfferRow->update($gider_satir_id, $offer_row_data);
                    }
                }

                echo json_encode(['icon' => 'success', 'message' => 'Gider başarıyla güncellendi.', 'errRows' => 0, 'offerId' => $gider_id]);
                return;
            } catch (\Exception $e) {
                $backtrace = $e->getTrace();
                $errorMessage = $e->getMessage();
            ...
            $offer_rows = $this->modelOfferRow->where('user_id', session()->get('user_id'))->where('gider_id', $gider_id)->findAll();
            $Kurlar = $this->modelMoneyUnit->findAll();
            ...
            return view('tportal/stoklar/giderler/duzenle', $data);
        }
    }
    ...
    public function delete($gider_id = null)
    {
        if ($this->request->getMethod('true') == 'POST') {
            try {
                $offer_item = $this->modelOffer->where('user_id', session()->get('user_id'))->where('gider_id', $gider_id)->first();

                if (!$offer_item) {
                    echo json_encode(['icon' => 'error', 'message' => 'Belirtilen Gider bulunamadı.']);
                    return;
                }   
            
                $Hareketler = $this->modelFinancialMovement->where('user_id', session()->get('user_id'))->where('gider_id', $gider_id)->findAll();
                foreach ($Hareketler as $value) {
                  $this->modelFinancialMovement->where('financial_movement_id', $value["financial_movement_id"])->delete();
                }
                $cari_id = $offer_item["cari_id"];
                ...
                $this->modelOffer->delete($offer_item['gider_id']);





                echo json_encode(['icon' => 'success', 'message' => 'Gider başarıyla silindi.']);
                return;
            } catch (\Exception $e) {
                $errorMessage = $e->getMessage();
                $errorLine = $e->getLine();
                ...
                echo json_encode([
                    'icon' => 'error',
                    'message' => "$errorMessage (File: $errorFile, Line: $errorLine)"
                ]);
                return;
                return;
            }
    ...
    public function teklifYazdir($invoice_id)
    {
        ...
        ->where('offer.gider_id', $invoice_id)
        ->orderBy('offer_row.gider_satir_id', "ASC")
