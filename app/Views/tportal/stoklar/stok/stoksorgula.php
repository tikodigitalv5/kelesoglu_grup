...
<?= $this->section('page_title') ?> Barkod Sorgulama | <?= $this->endSection() ?>
<?= $this->section('title') ?> Barkod Sorgulama | <?= session()->get('user_item')['firma_adi'] ?><?= $this->endSection() ?>
...
<div class="nk-content nk-content-fluid">
  ...
  <li class="nav-item">
    <a class="nav-link "  href="<?php echo route_to("tportal.stocks.depo_stoklari_say") ?>">
  ...
  <li class="nav-item">
  <a class="nav-link active "  href="<?php echo route_to("tportal.stocks.barkod_sorgula") ?>">
    <em class="icon ni ni-package"></em>
                        ...
                        <div class="card-inner position-relative card-tools-toggle">
                            <div class="card-title-group">
                                <div class="card-tools">
                                    <div class="">
                                    <h5>Barkod Sorgulama</h5>
                                    <p><b style="color: red; text-transform:uppercase;">Bu alanda okutulan barkodun alış ve satışlarını kolayca görebileceksiniz.</b></p>
...
function updateTotalStock() {
    ...
    let toplamStok = addedBarcodes.reduce((total, barcode) => {
        return total + (parseFloat(barcode.used_amount));
    }, 0);
...
function submitOnEnter(event) {
    if (event.key === "Enter") {
        ...
        $.ajax({
            url: '<?php echo route_to("tportal.stock.getBarkodDetail"); ?>',
            method: 'POST',
            dataType: 'json',
        ...
        if (fark !== 0) {
            Swal.fire({
                ...
                html: `
                    <table style="width: 100%; text-align: left; font-size: 16px;">
                        ...
                        <tr>
                            ...
                            <td style="text-align: right; font-weight:bold; color:green; font-family: monospace;">${fark.toFixed(2)}</td>
                        </tr>
                    </table>
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '<?php echo route_to("tportal.stock.barkodEkle"); ?>',  // Replace with your save route
                        method: 'POST',
                        dataType: 'json',
                        data: {
