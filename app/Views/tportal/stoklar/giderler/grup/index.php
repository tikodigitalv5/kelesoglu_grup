                            ...
                            <div class="nk-tb-list nk-tb-ulist">
                              ...
                              foreach($category_items as $category_item){ ?>
                                ...
                                <div class="nk-tb-item" data-url="'.route_to('tportal.stoklar.detay').'"
                                    id="category_'.$category_item['gider_group_category_id'].'">
                                    <div class="nk-tb-col nk-tb-col-check">
                                        <div class="custom-control custom-control-sm custom-checkbox notext">
                                    ...
                                    <div class="nk-tb-col">
                                        <div class="user-card">
                                            <div class="user-info">
                                                <span class="tb-lead"><?= $category_item['gider_group_category_title'] ?></span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="nk-tb-col tb-col-md">
                                        <div class="custom-control custom-switch ps-0">
                                            <div id="div_status_<?= $category_item['gider_group_category_id'] ?>"
                                                class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input status-button"
                                                    data-status="<?= $category_item['status'] ?>"
                                                    data-id="<?= $category_item['gider_group_category_id'] ?>"
                                                  
                                                    id="chk_status_<?= $category_item['gider_group_category_id'] ?>"
                                                    <?php if($category_item['status'] == 'active'){ ?>checked
                                                    <?php } ?>>
                                                <label class="custom-control-label"
                                                    id="btn_status_<?= $category_item['gider_group_category_id'] ?>"
                                                    for="chk_status_<?= $category_item['gider_group_category_id'] ?>">
                                    ...
                                    <div class="nk-tb-col nk-tb-col-tools  text-end">
                                        <a href="#" data-bs-toggle="modal" data-bs-target="#editModalForm"
                                            data-item-id="<?= $category_item['gider_group_category_id'] ?>"
                                            data-title="<?= $category_item['gider_group_category_title'] ?>"
                                        ...
                                        <a href="#"
                                            ...
                                            data-item-id="<?= $category_item['gider_group_category_id'] ?>"><em
            ...
            <div class="modal-body bg-white">
                <form onsubmit="return false;" id="createCategoryForm" method="post" class="form-validate is-alter">
                    <div class="form-group">
                        <label class="form-label" for="gider_group_category_title"> <?php echo $title_new; ?> Adı</label>
                        <div class="form-control-wrap">
                            <input type="text" class="form-control form-control-xl" id="gider_group_category_title"
                                name="gider_group_category_title" required>
                        </div>
                    </div>
                  
                    <div class="form-group">
                        <label class="form-label" for="gider_group_category_title">Sıralama</label>
                        <div class="form-control-wrap">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body bg-white">
                <form id="editCategoryForm" method="post" class="form-validate is-alter" enctype="multipart/form-data">
                    <div class="form-group">
                        <label class="form-label" for="edit_gider_group_category_title"> <?php echo $title_new; ?> Adı</label>
                        <div class="form-control-wrap">
                            <input type="text" class="form-control form-control-xl" id="edit_gider_group_category_title"
                                name="gider_group_category_title" required>
                        </div>
                    </div>
                    <div class="form-group">
...
$('#categoryOlustur').click(function(e) {
    if ($('#gider_group_category_title').val() == '') {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '<?= route_to('tportal.stocks.gider_gruplari.create') ?>',
            dataType: 'json',
            data: formData,
            async: true,
...
$('#modal_delete_category_button').on('click', function() {
    $.ajax({
        ...
        type: 'POST',
        url: '<?= route_to('tportal.stocks.gider_gruplari.delete') ?>',
        dataType: 'json',
        data: {
            gider_group_category_id: item_id,
        },
        async: true,
        success: function (response) {
...
$('.edit-action').on('click', function() {
    ...
    $('#edit_gider_group_category_title').val(item_title);
...
if ($('#edit_category_title').val() == '') {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '<?= route_to('tportal.stocks.gider_gruplari.edit') ?>',
        dataType: 'json',
        data: formData,
        async: true,
        success: function(response) {
...
$('.status-button').on('change', function() {
    ...
    gider_group_category_id = $(this).data('id');
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '<?= route_to('tportal.stocks.gider_gruplari.edit.status') ?>',
        dataType: 'json',
        data: {
            status: status,
            gider_group_category_id: gider_group_category_id
        },
        async: true,
        success: function(response) {
            if (response['icon'] == 'success') {
...
$('#close_modal_create').on('click', function(){
    gider_group_category_id = $(this).attr('data-category-id')
    $('#mdl_variant_category_list_'+gider_group_category_id).modal('toggle');
});
