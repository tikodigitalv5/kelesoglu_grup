...
<?php endif; ?>
        ...
        columns: [{
                ...
                render: function(data, type, row) {
                    ...
                    if(data.order_status === 'kargo_bekliyor' || data.order_status === 'sevk_edildi' || data.order_status === 'kargolandi') {
                        disabled = '';
                    }
                    ...
                    <div class="custom-control custom-control-sm custom-checkbox notext datatable-checkbox-container">
                        <input type="checkbox" class="custom-control-input datatable-checkbox" data-id="${data.order_id}" data-stock-no="${data.order_id}" id="stockRow_${data.order_id}" ${disabled}>
...
Swal.fire({
    title: 'Seçili olan siparişler için irsaliye oluşturmak istiyormusunuz?',
    icon: 'warning',
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
        Swal.fire({
        title: 'İrsaliye Taslak Oluşturuluyor lütfen sayfayı kapatmayınız!',
        allowOutsideClick: false,
        onBeforeOpen: () => {
            Swal.showLoading();
        },
    ...
    window.location.href = window.location.origin + '/tportal/siparisler/irsaliye?order_ids=' + selectedValues.join(',');
