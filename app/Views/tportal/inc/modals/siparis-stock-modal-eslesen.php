...
$(".modal-backdrop").remove();
        ...
        stockTable = $('#datatableStock').DataTable({
            processing: true,
            serverSide: true,
            deferLoading: 0, // İlk yüklemede veri çekilmesini engeller
            lengthMenu: [
                [10, 25, 50, -1],
                [10, 25, 50, "Tümü"]
            ],
            ajax: {
                ...
                data: function(d) {
                    d.search_value = $('#mdl_stock_searchs').val();
                        // Eğer arama kutusu 3 karakterden azsa, iptal için özel bir parametre ekle
                        if ($('#mdl_stock_searchs').val().length < 3) {
                            d.no_search = true;
                        }
                        return d;
                },
                dataSrc: function(json) {
                    // Eğer arama kutusu boşsa, veri döndürme
                    if ($('#mdl_stock_searchs').val().length < 3) {
                        return [];
                    }
                    return json.data;
            ...
            language: {
                emptyTable: "Arama yapmak için yukarıdaki kutuya en az 3 karakter giriniz.",
                zeroRecords: "Eşleşen kayıt bulunamadı",
                processing: '<div><div>Aranıyor</div><div><div></div><div></div><div></div></div></div>',
                search: "Ara:",
                paginate: {
                    first: "İlk",
                    last: "Son",
            ...
            },
            {
                ...
                render: function (data) {
                    ...
                    if(neredeyim != 'order' &&   neredeyim != 'production'  &&   neredeyim != 'offer'){
                            ...
                            var s_id = $("#mdl_urunSec").attr("data-s-id");

                            console.log("YENİ SATIR ID", s_id);
                        ...
                        <div class="custom-control custom-control-sm custom-checkbox notext datatable-checkbox-container">
                            <div class="custom-control custom-radio no-control">
                                <input type="radio" data-s_id="${s_id}" data-stock-code="${data.stock_code}" data-href="`+ base_part+ "/" + data.default_image + `" data-stockid="`+data.stock_id+`"  id="rd_stock_` + data.stock_id + `" name="rd_stock" class="custom-control-input rd_stock btnModalStockSiparis" value="` + data.stock_id + `" data-stock-code="`+data.stock_code+`" data-stock-parent-id="`+data.parent_id+`" data-stock-has-variant="`+data.has_variant+`"  data-stock-type="` + data.stock_type + `" data-stock-quantity="` + data.stock_total_quantity + `" data-stock-title="` + data.stock_title + `" data-stock-unit="` + data.sale_unit_id + `" data-stock-unit-price="` + parseFloat(data.sale_unit_price).toFixed(2) + `"  data-stock_ana_gorsel="` + data.default_image + `"  data-stock-warehouse-id="` + data.warehouse_id + `" data-stock-unit-sale-tax-rate="` + data.sale_tax_rate + ` ">
        ...
        $(document).on('keyup', '#mdl_stock_searchs', function() {
            ...
            searchTimeout = setTimeout(() => {
                ...
                if(searchValue && searchValue.length >= 3) {
                    ...
                    // 3 karakterden az girildiğinde mesaj göster
                    stockTable.clear().draw();
                }
            }, 500);
        });
    ...
    $(document).on("click", ".btnModalStockSiparis", function () {

var s_id = $(this).attr("data-s_id");

console.log("SİPARİS SEÇİLİRKEN SATİR ID", s_id);
var stockid = $(this).attr("data-stockid");
var stock_title = $(this).attr("data-stock-title");
var stock_code = $(this).attr("data-stock-code");
var default_image = $(this).attr("data-href");
console.log(default_image);



$("#img_urun_" + s_id).attr("src", default_image);
$("#imghref_" + s_id).attr("href", default_image);
$("#imghrefs_" + s_id).attr("data-href", default_image);
$("#imghrefs_" + s_id).attr("data-title", stock_title);
$("#img_urun_" + s_id).removeClass("d-none");
$(".img_urun_" + s_id).addClass("d-none");
$(".yeni_urun_" + s_id).val(stock_title + "\nÜrün Kodu : " + stock_code);
$(".stok_id_" + s_id).val(stockid);
