$(document).ready(function () {
    ...
    function fatura_satir_hesapla(gln) {
        ...
        if ($("#txt_iskonto_tutari_" + gln).val() == "") {
            ...
            var iskonto_tutari = $("#txt_iskonto_tutari_" + gln)
                .val()
                .replace(".", "")
                .replace(",", ".");
        }
        ...
        var k = 0;
        var aratplm_ = (birim_fiyat * miktar) - iskonto_tutari;

        if (aratplm_ < 0) {
            var focused = $(':focus');
            focused.val(0);
            swetAlert("Uyarı", "Ara toplam 0'dan küçük olamaz. <br> Lütfen girdiğiniz değerleri kontrol", "err");
            ...
            // var kdv_ = yuvarla(aratplm_ * k,2);
            var kdv_ = aratplm_ * k;


            //  console.log(format(2.005)); // "2.01"

            var tplm = aratplm_ + kdv_; //aratplm_ * (1 + k);
            ...
            if ($(".diger_vergi").attr("visible", "true")) {
                ...
                $("#txt_V9015_islem_tutar_" + gln).val(number_format(aratplm_, 5, ',', '.'));
                $("#txt_V9015_hesaplanan_kdv_" + gln).val(number_format(kdv_, 2, ',', '.'));
            }

            $("#txt_kdv_" + gln).val(number_format(kdv_, 2, ',', '.'));
            $("#txt_ara_toplam_" + gln).val(number_format(aratplm_, 6, ',', '.'));
